import numpy as np
from lib.math import svd_min, segment_point_distance as segment_d
...
def contours(
    ...
    min_eps_dist = step
    # min_eps_dist = .5 * min([ abs(a-eps[(i+1)%len(eps)]) for i, a in enumerate(eps) ]) if len(eps) > 1 else step
    print("eps", min_eps_dist)
        ...
        def is_bound(z):
            for B in bounds:
                ...
                for i, a in enumerate(B):
                    b = B[(i+1)%len(B)]
                    if segment_d(a, b, z) < min_eps_dist: return True
            return False
